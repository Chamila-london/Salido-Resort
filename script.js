...
(function(){
  ...
  /* gallery lightbox (null-guarded — only runs on pages that have a gallery) */
  var gal = document.querySelector('.lux-gallery');
  if(gal){
    var shots = [].slice.call(gal.querySelectorAll('figure'));
    ...
    var lbX = document.getElementById('lb-x'),
        lbP = document.getElementById('lb-p'),
        lbN = document.getElementById('lb-n');
    function show(i){
      cur = (i + shots.length) % shots.length;
      var img = shots[cur].querySelector('img');
      lbImg.src = img.src; lbImg.alt = img.alt || '';
      /* captions were removed from the markup — fall back to the image's alt text */
      var cap = shots[cur].querySelector('figcaption');
      lbCap.textContent = cap ? cap.textContent.trim() : (img.alt || '');
    }
    function open(i){ lastFocus = document.activeElement; show(i); lb.classList.add('open');
    ...
    /* simple focus trap: keep Tab inside the open lightbox */
  ...
  if(wx){
    ...
    var url = 'https://api.open-meteo.com/v1/forecast?latitude=7.2906&longitude=80.6337' +
    ...
    fetch(url).then(function(r){ if(!r.ok) throw 0; return r.json(); }).then(function(d){
      ...
      wx.innerHTML =
        '<div class="wx__ico">' + ICON[pick(c.weather_code)] + '</div>' +
        '<div class="wx__body"><span class="wx__loc">Kandy right now</span>' +
        '<span class="wx__temp">' + t + '\u00B0C</span>' +
        '<span class="wx__cond">' + (WCODE[c.weather_code] || '\u2014') + '</span>' +
        '<span class="wx__meta">H ' + hi + '\u00B0 \u00B7 L ' + lo + '\u00B0 \u00B7 Humidity ' + c.relative_humidity_2m + '%</span></div>';
